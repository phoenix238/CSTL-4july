...
export async function GET(req: Request) {
  try {
    ...
    // ?type=clean asks for 90-minute Clean Language slots; anything else is the
    // standard 60-minute session.
    const sessionType = parseSessionType(params.get("type"));
    const slots = await loadAvailableSlots({ clinic: clinic as Clinic, windowStart, windowEnd, sessionType });
